            ...
            <div className="flex gap-1 bg-[#18181B] p-1.5 rounded-full border border-gray-800 shadow-inner">
              {['all', 'kick', 'twitch', 'youtube'].map(p => (
                <button 
                  key={p} 
                  onClick={() => setFilter(p)} 
                  className={`px-6 py-2.5 rounded-full text-[10px] font-black transition-all uppercase tracking-widest ${
                    filter === p 
                      ? (p === 'kick' ? 'bg-[#53FC18] text-black shadow-[#53FC18]/20' : 
                         p === 'youtube' ? 'bg-[#FF0000] text-white shadow-[#FF0000]/20' : 
                         p === 'twitch' ? 'bg-[#9146FF] text-white shadow-[#9146FF]/20' : 
